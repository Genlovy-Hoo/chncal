...
class Arrangement(object):
    ...
    def __init__(self):
        ...
        ext_data = _load_ext_data().set_index('date')
        ext_data.sort_index(ascending=True, inplace=True)
        self.ext_data = ext_data['is_open'].to_dict()
...
def _load_ext_data():
    ...
    fpath = os.path.join(current_path, 'SSE.csv')
    if not os.path.exists(fpath):
        warnings.warn('A股交易日csv数据未找到！',
...
def generate_constants():
    ...
    ext_path = _get_constants_file_path('constants_atrade.py')
...
atrade_calendar = {
'''
    for date, isopen in arrangement.ext_data.items():
        ext_str += '    datetime.date(year={}, month={}, day={}): {},\n'.format(
            date[:4], int(date[5:7]), int(date[8:]), int(isopen))
    ext_str += '}'
